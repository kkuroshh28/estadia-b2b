 ...
 * Todo simulado: en producción esto conecta con el proveedor KYC.
 */
...
function Campo({ etiqueta, placeholder, tipo = "text" }: { etiqueta: string; placeholder: string; tipo?: string }) {
  return (
...
function Registro() {
  ...
  const alias = useMemo(() => generarAlias(), []);
  ...
  const avanzarDesdeBanco = () => setPaso(esComisionista ? 4 : 5);
            ...
            {paso === 1 && (
              <motion.div key="datos" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }}>
                ...
                <div className="mt-6 space-y-4">
                  <Campo etiqueta="Nombre completo" placeholder="Como aparece en tu cédula" />
                  <Campo etiqueta="Cédula" placeholder="1.234.567.890" />
                  <Campo etiqueta="Celular" placeholder="300 000 0000" tipo="tel" />
                  <Campo etiqueta="Correo" placeholder="tu@correo.com" tipo="email" />
                </div>
