...
import { secretoObligatorio, simuladorPermitido } from "@/server/config";
...
export async function POST(req: Request) {
  ...
  if (!simuladorPermitido() || (process.env.KYC_DRIVER ?? "simulado") !== "simulado") {
  ...
  const peticion = new Request("http://interno/api/kyc/callback", {
    method: "POST",
    headers: { "x-firma-kyc": hmacFirma(cuerpo, secretoObligatorio("KYC_CALLBACK_SECRET") ?? "dev-kyc-secret") },
    body: cuerpo,
  });
