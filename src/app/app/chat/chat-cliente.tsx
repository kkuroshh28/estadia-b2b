...
export function ChatCliente({ datos }: { datos: DatosChat }) {
  ...
  const enviar = async () => {
    ...
    if (!datos.solicitudId) {
      ...
      // Vitrina: el veredicto viene del MISMO endpoint server-side (sin DB no
      // persiste); strikes y ban se simulan localmente para mostrar la regla.
            ...
            <input
              ...
              placeholder={
                baneado
                  ? "Cuenta suspendida"
                  : datos.esDemo
                    ? 'Prueba escribir "mi número es 310 555 1234"…'
                    : "Escribe tu mensaje…"
              }
